...
var EventItem = (_temp = _class = function (_Component) {
        ...
        value: function render() {
            ...
            var eventItemTemplate = _react2.default.createElement(
                ...
                { className: roundCls + ' event-item', key: eventItem.id,
                    style: { height: config.eventItemHeight, background: bgColor,marginTop: config.eventItemLineHeight/2-config.eventItemHeight/2 } },
                _react2.default.createElement(
                    'span',
                    { style: {  lineHeight: config.eventItemHeight + 'px' } },
                    eventTitle
                )
            );
            ...
            var a = _react2.default.createElement(
                'div',
                { className: 'timeline-event', style: { left: left, width: width, top: top, pointerEvents:"none" }, onClick: function onClick() {
    ...
    this.stopStartDrag = function (ev) {
        ...
        let conflictPosition=undefined;
        ...
        if (hasConflict) {
            ...
            if (conflictOccurred != undefined) {
                conflictOccurred(schedulerData, 'StartResize',events[conflictPosition], eventItem);
            } else {
    ...
    this.stopEndDrag = function (ev) {
        ...
        if (hasConflict) {
            ...
            if (conflictOccurred != undefined) {
                conflictOccurred(schedulerData, 'EndResize',events[conflictPosition], eventItem);
            } else {
                console.log('Conflict occurred, set conflictOccurred func in Scheduler to handle it');
            }
            _this3.subscribeResizeEvent(_this3.props);
